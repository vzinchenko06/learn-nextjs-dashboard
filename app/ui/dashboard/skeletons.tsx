...
export function TableRowSkeleton ({ cols }: { cols: number }) {
  return (
    <tr
      ...
      {Array(cols).fill('col').map((value, index) => (
        <td className="whitespace-nowrap px-3 py-3" key={`${value}-${index}`}>
...
export function TableSkeleton ({ cols, rows = 6 }: { cols: number | string[], rows?: number }) {
  return (
      ...
      <div className="inline-block min-w-full align-middle">
        <div className="rounded-lg bg-gray-50 p-2 md:pt-0">
          <div className="md:hidden">
            {Array(rows).fill('row').
              map((value, index) => (
                <TableMobileSkeleton key={`${value}-${index}`}/>
              ))}
          </div>
          <table className="hidden min-w-full text-gray-900 md:table">
            <thead className="rounded-lg text-left text-sm font-normal">
            <tr>
              {(Array.isArray(cols) ? cols : Array(cols).fill('col')).map(
                (value, index) =>
                  value ? (
                    <th key={`${value}-${index}`} scope="col"
                    ...
                    <th key={`${value}-${index}`} scope="col"
                      ...
                      <span className="sr-only">Column-{index}</span>
                    </th>
                  ),
              )}
            </tr>
            ...
            {Array(rows).
              map((value, index) => (
                <TableRowSkeleton
                  key={`${value}-${index}`}
                  cols={Array.isArray(cols) ? cols.length : cols}
                />
              ))}
            </tbody>
          </table>
